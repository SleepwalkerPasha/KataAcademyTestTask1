...
## Реализация решения:
Для проверки строки на условие наличия в ней арабских цифр использовал регулярные выражения. Для проверки на наличие римских - сплитил строку с сепаратором в виде оператора и проверял полученный массив, римские ли в нем или нет. Для перевода римских в целые и обратно разработал методы intToRoman(int num), romanToInt(String str). Метод valueCheck() проверяет на корректность ввода(2 целых или 2 римских), считая, сколько было введено римских 
Реализация калькулятора достаточно проста, использовал switch конструкции.
